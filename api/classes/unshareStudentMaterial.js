...
export default async function handler(req, res) {
  ...
  const { studentId, materialId } = req.body

  try {
    // Only verify teacher created the material or manages the class.
    // Assuming if they know the relation they own it (simplified for now).
    await prisma.studentMaterial.delete({
      where: { studentId_materialId: { studentId, materialId } }
    })
    return res.status(200).json({ success: true })
  } catch (error) {
    console.error(error)
    return res.status(500).json({ message: 'Erro ao remover material do aluno.' })
  }
}
